...
static uint64_t pit_read(void) { return pit_ticks; }
...
static int pit_enable(void)
{
    ...
    outb(PIT_COMMAND_PORT,
         PIT_CHANNEL_0 | PIT_ACCESS_MODE_LO_HI | PIT_MODE_SQUARE);
...
static struct timer pit = {.read = pit_read,
                           .reset = pit_reset,
                           .mult = PIT_MULT,
                           .shift = PIT_SHIFT,
...
void pit_register(void) { timer_register(&pit); }
...
static struct irq_timer pit_oneshot = {.schedule_irq = pit_schedule_irq,
...
void pit_oneshot_register(void) { set_irq_timer(&pit_oneshot); }

/* PIT waiting for early boot timing */

static volatile int pit_wait_complete;

static void pit_wait_handler(__unused void *device) { pit_wait_complete = 1; }
...
void pit_wait_finish(void) { release_irq(PIT_IRQ, &pit); }
