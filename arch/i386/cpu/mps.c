...
static void __mp_processor(struct mp_table_processor *s)
{
    if (s->cpu_flags & MP_PROCESSOR_ACTIVE) {
        if (!lapic_add(s->apic_id)) {
            klog(KLOG_WARNING,
                 MPS "maximum number of CPUs reached, ignoring lapic %d",
                 s->apic_id);
            return;
        }
    }
    ...
    klog(KLOG_INFO,
         MPS "LAPIC id %d %sactive",
         s->apic_id,
         s->cpu_flags & MP_PROCESSOR_ACTIVE ? "" : "in");
}
...
static void __mp_bus(struct mp_table_bus *s)
{
    ...
    klog(KLOG_INFO, MPS "bus id %d signature %.6s", s->bus_id, s->bus_type);
}
...
static void __mp_ioapic(struct mp_table_io_apic *s)
{
    ...
    klog(KLOG_INFO,
         MPS "I/O APIC id %d base %p irq_base %d",
         s->ioapic_id,
         s->ioapic_base,
    ...
    if (!ioapic) {
        klog(KLOG_WARNING,
             MPS "maximum supported number of I/O APICs reached, ignoring");
    } else {
...
static void __mp_io_interrupt(struct mp_table_io_interrupt *s)
{
    ...
    if (s->dest_ioapic == 0xFF) {
        ...
        if (ioapics_available != 1) {
            klog(KLOG_ERROR, MPS "ignoring I/O INT for pin %d", s->dest_intin);
            return;
        }
    ...
    case MP_INTERRUPT_TYPE_INT:
        ...
        case BUS_TYPE_UNKNOWN:
            ioapic_set_bus(ioapic, s->dest_intin, mp_buses[s->source_bus]);
            break;
        default:
            klog(KLOG_ERROR,
    ...
    default:
        klog(KLOG_ERROR,
             MPS "ignoring unknown I/O INT type %d",
             s->interrupt_type);
        return;
    }
    ...
    klog(KLOG_INFO,
         MPS "I/O INT bus %d int %d ioapic %d pin %d type %s",
         s->source_bus,
         s->source_irq,
         s->dest_ioapic,
         s->dest_intin,
...
static void __mp_local_interrupt(struct mp_table_local_interrupt *s)
{
    ...
    case MP_INTERRUPT_TYPE_INT:
        klog(KLOG_WARNING,
             MPS "ignoring LOC INT irq %d for LINT%d",
             s->source_irq,
             s->dest_lintin);
        type = "INT";
        break;
    ...
    default:
        klog(KLOG_ERROR,
             MPS "ignoring unknown LOC INT type %d",
             s->interrupt_type);
        return;
    }
    ...
    klog(KLOG_INFO,
         MPS "LOC INT bus %d int %d lapic %d LINT%d type %s",
         s->source_bus,
         s->source_irq,
         s->dest_lapic,
         s->dest_lintin,
