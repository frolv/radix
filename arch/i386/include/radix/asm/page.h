...
static __always_inline pde_t make_pde(pdeval_t val) { return (pde_t){val}; }

static __always_inline pte_t make_pte(pteval_t val) { return (pte_t){val}; }
...
#define __arch_cache_flush_all() \
    do {                         \
    } while (0)
#define __arch_cache_flush_page(addr) \
    do {                              \
    } while (0)
