...
static void __madt_lapic(struct acpi_madt_local_apic *s)
{
    if (s->flags & ACPI_MADT_LOCAL_APIC_ACTIVE) {
        if (!lapic_add(s->apic_id)) {
            klog(KLOG_WARNING,
                 ACPI "maximum number of CPUs reached, ignoring lapic %d",
                 s->apic_id);
            return;
        }
    }
    ...
    klog(KLOG_INFO,
         ACPI "LAPIC id %d %sactive",
         s->apic_id,
         s->flags & ACPI_MADT_LOCAL_APIC_ACTIVE ? "" : "in");
}
...
static void __madt_ioapic(struct acpi_madt_io_apic *s)
{
    ...
    klog(KLOG_INFO,
         ACPI "I/O APIC id %d base %p irq_base %d",
         s->id,
         s->address,
         s->global_irq_base);
}
...
static void __madt_override(struct acpi_madt_interrupt_override *s)
{
    ...
    if (!ioapic) {
        klog(KLOG_ERROR,
             ACPI "ignoring ISA IRQ override for invalid irq %d",
             s->irq_source);
        return;
    }
    ...
    klog(KLOG_INFO,
         ACPI "IRQ override bus %d int %d ioapic %d pin %d",
         s->bus_source,
         s->irq_source,
         ioapic->id,
         pin);
...
static void __madt_nmi(struct acpi_madt_nmi_source *s)
{
    ...
    if (!ioapic) {
        klog(KLOG_ERROR, ACPI "ignoring NMI for invalid irq %d", s->global_irq);
        return;
    }
