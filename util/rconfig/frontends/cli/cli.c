...
static struct option long_opts[] = {{"arch", required_argument, NULL, 'a'},
...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt_long(argc, argv, "a:dhlo:", long_opts, NULL)) != EOF) {
        switch (c) {
        case 'a':
            arch = optarg;
    ...
    if ((err = rconfig_verify_src_dirs(&errdir)) != 0) {
        ...
        case EINVAL:
            fprintf(
                stderr, "%s: invalid or unsupported architecture\n", argv[0]);
            break;
        default:
            fprintf(stderr, "%s: %s\n", errdir, strerror(err));
            fprintf(
                stderr, "%s: are you in the radix root directory?\n", argv[0]);
            break;
        }
        return 1;
    }
