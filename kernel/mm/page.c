...
uint64_t totalmem(void) { return memsize; }

uint64_t usedmem(void) { return memused; }
...
#define IN_RANGE(mmap, mbt) ((addr_t)mmap < mbt->mmap_addr + mbt->mmap_length)
...
static size_t zone_init(size_t pfn,
                        size_t section_end,
                        struct buddy *zone,
                        unsigned int flags);
    ...
    while (pfn < section_end) {
        ...
        for (; pfn < end; ++pfn) {
            ...
            if (flags & PM_PAGE_MAPPED)
                page_map[pfn].mem = (void *)phys_to_virt(pfn << PAGE_SHIFT);
